...
commentSchema.statics={    
    ...
    findByMovie:function(movieId,callback){
        return this.find({movie:movieId}).sort({'meta.created_at':-1}).limit(3).populate('from','user').exec(callback);
    },
    //查看当前电影下更多评论的方法
    findMoreByMovie:function(movieId,from,limit,callback){
        return this.find({movie:movieId}).skip(from).sort({'meta.created_at':-1}).limit(limit).populate('from','user').exec(callback);
    },
