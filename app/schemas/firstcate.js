...
//设计firstcate数据表结构
var firstcateSchema=new mongoose.Schema({
    name:{
        unique:true,
        type:String
    },
 ...
 * 第一个参数是要为哪个操作设置回调，
 * 第二个参数就是要设置的回调方法
 * 为firstcateSchema设置保存数据的方法，
 * 当保存的数据是新添加的时候，
 * 更新created_at和updated_at都为当前时间，
 * 如果数据不是新添加的是新修改的，
 * 则只更新updated_at为当前时间
 */
firstcateSchema.pre('save',function(next){
    /*
...
/*
 * 通过module.exports将firstcateSchema对外公开，
 * 这样外部就可以访问到firstcateSchema
 */
module.exports=firstcateSchema;
